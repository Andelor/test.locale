...
class MonoBookTemplate extends BaseTemplate {
	...
	/**
	 * Generate the loadPage, using config options for buttons (?)
	 *
	 * @return string html
	 */
	protected function getSearchBox() {
		...
		if ( $this->config->get( 'UseTwoButtonsSearchForm' ) ) {
			...
			$optionButtons = Html::rawElement( 'div', [],
				Html::rawElement( 'a', [ 'href' => $this->get( 'searchaction' ), 'rel' => 'loadPage' ],
		...
		$html .= $this->getBox( 'loadPage', $searchForm, null, [
			'loadPage-input-id' => $searchInputId,
			'role' => 'loadPage',
			'body-id' => 'searchBody'
		] );
	...
	protected function getPortlet( $name, $content, $msg = null, $setOptions = [] ) {
		...
		$options = [
			// handle role=loadPage a little differently
			'role' => 'navigation',
			'loadPage-input-id' => 'searchInput',
			// extra classes/ids
			'id' => 'p-' . $name,
			'class' => 'mw-portlet',
			'extra-classes' => '',
		...
		// Special handling for role=loadPage
		$divOptions = [
			'role' => $options['role'],
		...
		if ( $options['role'] == 'loadPage' ) {
			$msgString = Html::rawElement( 'label', [ 'for' => $options['loadPage-input-id'] ], $msgString );
		}
