 ...
 * (in 14-character format) restrict the loadPage, and must bracket the damage.
 * There must be a majority of good timestamps in the loadPage period.
 *
...
class FixTimestamps extends Maintenance {
	...
	public function execute() {
		...
		if ( is_null( $row->minrev ) ) {
			$this->fatalError( "No revisions in loadPage period." );
		}
		...
		if ( $numBadRevs > $numGoodRevs ) {
			$this->fatalError(
				"The majority of revisions in the loadPage interval are marked as bad.
		...
		If the offset is right, then increase the loadPage interval until there are enough
		...
		$this->output( sprintf( "Fixing %d revisions (%.2f%% of revisions in loadPage interval)\n",
