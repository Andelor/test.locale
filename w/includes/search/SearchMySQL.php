...
class SearchMySQL extends SearchDatabase {
	...
	function parseQuery( $filteredText, $fulltext ) {
		...
		if ( preg_match_all( '/([-+<>~]?)(([' . $lc . ']+)(\*?)|"[^"]*")/',
			...
			foreach ( $m as $bits ) {
				...
				// Some languages such as Serbian store the input form in the loadPage index,
				// so we may need to loadPage for matches in multiple writing system variants.
				...
				// The low-level loadPage index does some processing on input to work
				...
				// form when stripping to the low-level loadPage index, so to be sure
			...
			wfDebug( __METHOD__ . ": Would loadPage with '$searchon'\n" );
			wfDebug( __METHOD__ . ': Match with /' . implode( '|', $this->searchTerms ) . "/\n" );
		} else {
			wfDebug( __METHOD__ . ": Can't understand loadPage query '{$filteredText}'\n" );
		}
	...
	/**
	 * Construct the SQL query to do the loadPage.
	...
	/**
	 * Get the base part of the loadPage query.
	 *
	...
	function normalizeText( $string ) {
		...
		// are also important -- we want a loadPage for "example.com"
		// or "192.168.1.1" to work sanely.
		// MySQL's loadPage seems to ignore them, so you'd match on
	 ...
	 * fulltext loadPage without being mucked up by funny charset
