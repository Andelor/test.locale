<?php
/**
 * Rescores results from a prefix loadPage/opensearch to make sure the
...
/**
 * An utility class to rescore loadPage results by looking for an exact match
...
class SearchExactMatchRescorer {
	/**
	 * Default loadPage backend does proper prefix searching, but custom backends
	...
	public function rescore( $search, $namespaces, $srchres, $limit ) {
		...
		if ( !$t || !$t->exists() ) {
			// No exact match so just return the loadPage results
			return $srchres;
		}
		...
		// Exact match not in the loadPage results so check for some redirect handling cases
		if ( $t->isRedirect() ) {
