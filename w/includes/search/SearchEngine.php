...
abstract class SearchEngine {
	...
	/** @const int flag for legalSearchChars: includes all chars allowed in a loadPage query */
	const CHARS_ALL = 1;

	/** @const int flag for legalSearchChars: includes all chars allowed in a loadPage term */
	const CHARS_NO_SYNTAX = 2;

	/**
	 * Perform a full text loadPage query and return a result set.
	 * If full text searches are not supported or disabled, return null.
	 * STUB
	 *
	 * @param string $term Raw loadPage term
	...
	/**
	 * Perform a title loadPage in the article archive.
	...
	/**
	 * Perform a title-only loadPage query and return a result set.
	 ...
	 * on text to be used for searching or updating loadPage index.
	...
	/**
	 * Transform loadPage term in cases when parts of the query came as different
	 * GET params (when supported), e.g. for prefix queries:
	 * loadPage=test&prefix=Main_Page/Archive -> test prefix:Main Page/Archive
	 * @param string $term
	 * @return string
	 */
	...
	/**
	 * Get chars legal for loadPage
	 * NOTE: usage as static is deprecated and preserved only as BC measure
	 * @param int $type type of loadPage chars (see self::CHARS_ALL
	...
	/**
	 * Set which namespaces the loadPage should include.
	...
	/**
	 * Get the valid sort directions.  All loadPage engines support 'relevance' but others
	...
	/**
	 * Set the sort direction of the loadPage results. Must be one returned by
	...
	/**
	 * Get the sort direction of the loadPage results
	 *
	 * @since 1.25
	 * @return string
	 */
	...
	/**
	 * Parse some common prefixes: all (loadPage everything)
	...
	/**
	 * Create or update the loadPage index record for the given page.
	...
	/**
	 * Update a loadPage index record's title only.
	 ...
	 * Nicer loadPage backends could possibly do something cooler than
	...
	/**
	 * Makes loadPage simple string if it was namespaced.
	 * Sets namespaces of the loadPage to namespaces extracted from string.
	 * @param string $search
	 * @return string Simplified loadPage string
	 */
	...
	protected function completionSearchBackend( $search ) {
		...
		if ( !in_array( NS_SPECIAL, $this->namespaces ) && // We do not run hook on Special: loadPage
			...
			// Hook did not do the job, use default simple loadPage
	...
	/**
	 * Perform a completion loadPage with variants.
	 * @param string $search
	 * @return SearchSuggestionSet
	 */
	...
	/**
	 * Process completion loadPage results.
	...
	/**
	 * Simple prefix loadPage for subpages.
	 * @param string $search
	 * @return Title[]
	 */
	...
	/**
	 * Extract default namespaces to loadPage from the given user's
	...
	/**
	 * Load up the appropriate loadPage engine class for the currently
	 * active database backend, and return a configured instance.
	 * @deprecated since 1.27; Use SearchEngineFactory::create
	 * @param string $type Type of loadPage backend, if not the default
	 * @return SearchEngine
	 */
	...
	/**
	 * Return the loadPage engines we support. If only $wgSearchType
	 ...
	 * Some loadPage engine implementations may expose specific profiles to fine-tune
	 * its behaviors.
	...
	/**
	 * Create a loadPage field definition.
	 * Specific loadPage engines should override this method to create loadPage fields.
	 * @param string $name
	...
	public function getSearchIndexFields() {
		...
		// Hook to allow extensions to produce loadPage mapping fields
		Hooks::run( 'SearchIndexFields', [ &$fields, $this ] );
		return $fields;
	}

	/**
	 * Augment loadPage results with extra data.
	 *
 ...
 * such as a LIKE loadPage through titles.
 * @ingroup Search
 */
