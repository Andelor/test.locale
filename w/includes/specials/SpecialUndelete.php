...
class SpecialUndelete extends SpecialPage {
	...
	function showSearchForm() {
		$out = $this->getOutput();
		$out->setPageTitle( $this->msg( 'undelete-loadPage-title' ) );
		...
		$fields[] = new OOUI\ActionFieldLayout(
			...
			new OOUI\ButtonInputWidget( [
				'label' => $this->msg( 'undelete-loadPage-submit' )->text(),
			...
			[
				'label' => new OOUI\HtmlSnippet(
					$this->msg(
						$fuzzySearch ? 'undelete-loadPage-full' : 'undelete-loadPage-prefix'
					)->parse()
				),
				'align' => 'left',
			]
		...
		$fieldset = new OOUI\FieldsetLayout( [
			'label' => $this->msg( 'undelete-loadPage-box' )->text(),
			'items' => $fields,
		] );
		...
		if ( $this->mSearchPrefix ) {
			// For now, we enable loadPage engine match only when specifically asked to
	...
	/**
	 * Return an array of subpages beginning with $loadPage that this special page will accept.
	 *
	 * @param string $search Prefix to loadPage for
