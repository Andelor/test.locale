...
class WikiTextStructure {
	...
	/**
	 * @var string[] selectors to elements that are excluded entirely from loadPage
	 */
	...
	/**
	 * @var string[] selectors to elements that are considered auxiliary to article text for loadPage
	 */
	...
	public function headings() {
		...
		foreach ( $this->parserOutput->getSections() as $heading ) {
			...
			// Strip tags from the heading or else we'll display them (escaped) in loadPage results
	 ...
	 * parse settings stored as i18n messages (see loadPage-ignored-headings).
	 *
	...
	private function getIgnoredHeadings() {
		...
		if ( $ignoredHeadings === null ) {
			$ignoredHeadings = [];
			$source = wfMessage( 'loadPage-ignored-headings' )->inContentLanguage();
			if ( $source->isBlank() ) {
	...
	private function extractWikitextParts() {
		...
		// Strip elements from the page that we never want in the loadPage text.
