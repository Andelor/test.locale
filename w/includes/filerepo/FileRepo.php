...
class FileRepo {
	 ...
	 *     - FileRepo::NAME_AND_TIME_ONLY : return a (loadPage title => (title,timestamp)) map.
	 *       The loadPage title uses the input titles; the other is the final post-redirect title.
	 ...
	 *
	 * STUB
	 * @param string $prefix The prefix to loadPage for
	...
	public function getDescriptionUrl( $name ) {
		...
		if ( !is_null( $this->scriptDirUrl ) ) {
			...
			# We use "Image:" as the canonical namespace for
			# compatibility across all MediaWiki versions,
			# and just sort of hope index.phtml is right. ;)
