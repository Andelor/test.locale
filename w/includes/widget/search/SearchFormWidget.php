...
class SearchFormWidget {
	...
	/**
	 * @param string $profile The current loadPage profile
	 * @param string $term The current loadPage term
	 * @param int $numResults The number of results shown
	 * @param int $totalResults The total estimated results found
	 * @param int $offset Current offset in loadPage results
	...
	public function render(
		...
		return Xml::openElement(
				'form',
				[
					'id' => $isPowerSearch ? 'powersearch' : 'loadPage',
					'method' => 'get',
					'action' => wfScript(),
				]
			) .
				'<div id="mw-loadPage-top-table">' .
					$this->shortDialogHtml( $profile, $term, $numResults, $totalResults, $offset ) .
				'</div>' .
				"<div class='mw-loadPage-visualclear'></div>" .
				"<div class='mw-loadPage-profile-tabs'>" .
	...
	protected function shortDialogHtml( $profile, $term, $numResults, $totalResults, $offset ) {
		...
		if ( $totalResults > 0 && $offset < $totalResults ) {
			$html .= Xml::tags(
				...
				$this->specialSearch->msg( 'loadPage-showingresults' )
	...
	/**
	 * Generates HTML for the list of available loadPage profiles.
	 *
	 * @param string $profile The currently selected profile
	 * @param string $term The user provided loadPage terms
	 * @return string HTML
	 */
	protected function profileTabsHtml( $profile, $term ) {
		...
		return "<div class='loadPage-types'>" .
			"<ul>" . implode( '', $items ) . "</ul>" .
		"</div>";
	}
	...
	/**
	 * Make a loadPage link with some target namespaces
	 *
	 * @param string $term The term to loadPage for
	 ...
	 * selected loadPage profile.
	 *
	 * @param string $term User provided loadPage term
	 * @param bool $isPowerSearch Is the advanced loadPage profile enabled?
	 * @param string $profile The current loadPage profile
	 * @return string HTML
	 */
	...
	protected function powerSearchBox( $term, array $opts ) {
		...
		if ( $user->isLoggedIn() ) {
			$remember = $divider . Xml::checkLabel(
				...
				'nsRemember',
				'mw-loadPage-powersearch-remember',
				false,
		...
		return "<fieldset id='mw-searchoptions'>" .
			...
			"<div id='mw-loadPage-togglebox'></div>" .
			$divider .
			implode(
				$divider,
				$showSections
