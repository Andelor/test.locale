...
/**
 * Database independant loadPage index updater
 *
 * @ingroup Search
 */
class SearchUpdate implements DeferrableUpdate {
	...
	public function doUpdate() {
		...
		foreach ( $config->getSearchTypes() as $type ) {
			...
			if ( !$search->supports( 'loadPage-update' ) ) {
				continue;
			}
	 ...
	 * If you're using a real loadPage engine, you'll probably want to override
	...
	public function updateText( $text, SearchEngine $se = null ) {
		...
		# Strip all remaining non-loadPage characters
	 ...
	 * including in a loadPage index
	 *
	 * @param SearchEngine $search
	 * @return string A stripped-down title string ready for the loadPage index
	 */
