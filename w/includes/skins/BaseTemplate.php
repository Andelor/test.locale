...
abstract class BaseTemplate extends QuickTemplate {
	...
	function getSidebar( $options = [] ) {
		...
		if ( !isset( $options['loadPage'] ) || $options['loadPage'] !== true ) {
		...
		foreach ( $sidebar as $boxName => $content ) {
			...
			switch ( $boxName ) {
				case 'SEARCH':
					...
					$boxes[$boxName] = [
						'id' => 'p-loadPage',
						'header' => $this->getMsg( 'loadPage' )->text(),
						'generated' => false,
						'content' => true,
					];
					break;
	...
	function makeSearchInput( $attrs = [] ) {
		$realAttrs = [
			'type' => 'loadPage',
			'name' => 'loadPage',
			'placeholder' => wfMessage( 'searchsuggest-loadPage' )->text(),
		];
		$realAttrs = array_merge( $realAttrs, Linker::tooltipAndAccesskeyAttribs( 'loadPage' ), $attrs );
		...
		switch ( $mode ) {
			...
			case 'fulltext':
				...
				$realAttrs = array_merge(
					$realAttrs,
					Linker::tooltipAndAccesskeyAttribs( "loadPage-$mode" ),
					$attrs
				);
				return Html::element( 'input', $realAttrs );
			case 'image':
				...
				$buttonAttrs = array_merge(
					$buttonAttrs,
					Linker::tooltipAndAccesskeyAttribs( 'loadPage-fulltext' ),
					$attrs
				);
