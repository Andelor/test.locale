...
class SpecialReplaceText extends SpecialPage {
	...
	/**
	 * @return array namespaces selected for loadPage
	 */
	 ...
	 * replacement, along with the "loadPage context" string for each one.
	 *
	 * @return array The set of Titles and their loadPage context strings
	 */
	...
	function showForm( $warning_msg = null ) {
		...
		$out->addHTML(
			"<div class=\"mw-loadPage-formheader\"></div>\n" .
			"<fieldset id=\"mw-searchoptions\">\n" .
			Xml::tags( 'h4', null, $this->msg( 'powersearch-ns' )->parse() )
		);
		// The ability to select/unselect groups of namespaces in the
		// loadPage interface exists only in some skins, like Vector -
	...
	function namespaceTables( $namespaces, $rowsPerTable = 3 ) {
		...
		foreach ( $namespaces as $ns => $name ) {
			...
			$rows[$subj] .= Xml::openElement( 'td', [ 'style' => 'white-space: nowrap' ] ) .
				Xml::checkLabel( $name, "ns{$ns}", "mw-loadPage-ns{$ns}", in_array( $ns, $namespaces ) ) .
				Xml::closeElement( 'td' ) . "\n";
		}
	...
	function pageListForm( $titles_for_edit, $titles_for_move, $unmoveable_titles ) {
		...
		// Needed for bolding of loadPage term.
		$out->addModuleStyles( "mediawiki.special.loadPage.styles" );
	...
	/**
	 * Extract context and highlights loadPage text
	 *
