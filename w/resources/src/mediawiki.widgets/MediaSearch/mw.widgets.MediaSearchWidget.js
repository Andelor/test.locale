...
( function ( $, mw ) {
	...
	mw.widgets.MediaSearchWidget.prototype.onResultsRemove = function ( items ) {
		if ( items.length > 0 ) {
			// In the case of the media loadPage widget, if any items are removed
			// all are removed (new loadPage)
			this.resetRows();
			this.currentItemCache = [];
		}
	};
	...
	/**
	 * Set language for the loadPage results.
	 *
	...
	/**
	 * Get language for the loadPage results.
	 *
