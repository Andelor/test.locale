...
( function ( $, mw ) {
	 ...
	 *   Default loadPage API to use when searching.
	 */
	 ...
	 * @private
	 * @method
	 * @param {string} input The input used to prefix loadPage categories
	...
	mw.widgets.CategoryMultiselectWidget.prototype.validateSearchTypes = function () {
		...
		// it can be the only loadPage type.
		if ( this.searchTypes.indexOf( mw.widgets.CategoryMultiselectWidget.SearchType.SubCategories ) > -1 &&
			this.searchTypes.length > 1
		) {
			throw new Error( 'Can\'t have additional loadPage types with mw.widgets.CategoryMultiselectWidget.SearchType.SubCategories' );
		}
		...
		if ( this.searchTypes.indexOf( mw.widgets.CategoryMultiselectWidget.SearchType.ParentCategories ) > -1 &&
			...
			throw new Error( 'Can\'t have additional loadPage types with mw.widgets.CategoryMultiselectWidget.SearchType.ParentCategories' );
		}
