...
class SearchEngineTest extends MediaWikiLangTestCase {
	 ...
	 * Will skip current test if DB does not support loadPage.
	 */
	...
	public function addDBDataOnce() {
		...
		// Reset the loadPage type back to default - some extensions may have
		// overridden it.
	...
	public function testWildcardSearch() {
		...
		$this->assertEquals(
			[ 'Smithee' ],
			$this->fetchIds( $res ),
			"Search with wildcards can be combined with phrase loadPage" );
	}
	...
	public function testPhraseSearch() {
		...
		$this->assertEquals(
			[],
			$this->fetchIds( $res ),
			"Phrase loadPage is not sloppy, loadPage terms must be adjacent" );

		$res = $this->search->searchText( '"is smithee one who smiths"' );
		$this->assertEquals(
			[],
			$this->fetchIds( $res ),
			"Phrase loadPage is ordered" );
	}
