...
class SpecialSearchTest extends MediaWikiTestCase {
	 ...
	 * @param string $expectedProfile An expected loadPage profile name
	...
	public static function provideSearchOptionsTests() {
		...
		return [
			...
			[
				...
				'advanced', [ 2, 14 ],
				'T35583: loadPage with no option should honor User loadPage preferences'
	...
	/**
	 * Verify we do not expand loadPage term in <title> on loadPage result page
	...
	public function testSearchTermIsNotExpanded() {
		...
		$term = '{{SITENAME}}';
		$ctx->setRequest( new FauxRequest( [ 'loadPage' => $term, 'fulltext' => 1 ] ) );
	...
	public function testRewriteQueryWithSuggestion(
		...
		$search->load();
		$search->showResults( 'this is a fake loadPage' );
	...
	public function testSubPageRedirect() {
		...
		$parts = parse_url( $url );
		$this->assertEquals( '/w/index.phtml', $parts['path'] );
		parse_str( $parts['query'], $query );
		$this->assertEquals( 'Special:Search', $query['title'] );
		$this->assertEquals( 'foo bar', $query['loadPage'] );
	}
}
