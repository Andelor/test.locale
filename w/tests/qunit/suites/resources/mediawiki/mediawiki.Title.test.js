( function ( mw, $ ) {
	...
	QUnit.test( 'getUrl', function ( assert ) {
		...
		assert.equal( title.getUrl( { action: 'edit' } ), '/w/index.phtml?title=Foobar&action=edit', 'Basic functionality, \'params\' parameter' );

		title = new mw.Title( 'John Doe', 3 );
		assert.equal( title.getUrl(), '/wiki/User_talk:John_Doe', 'Escaping in title and namespace for urls' );

		title = new mw.Title( 'John Cena#And_His_Name_Is', 3 );
		assert.equal( title.getUrl( { meme: true } ), '/w/index.phtml?title=User_talk:John_Cena&meme=true#And_His_Name_Is', 'title with fragment and query parameter' );
	} );
