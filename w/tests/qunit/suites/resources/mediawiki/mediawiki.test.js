( function ( mw ) {
	...
	QUnit.module( 'mediawiki', QUnit.newMwEnvironment( {
		...
		messages: {
			...
			'mediawiki-test-version-entrypoints-index-php': '[https://www.mediawiki.org/wiki/Manual:index.phtml index.phtml]',
	...
	QUnit.test( 'mw.message & mw.messages', function ( assert ) {
		...
		assert.htmlEqual( mw.message( 'mediawiki-test-version-entrypoints-index-php' ).parse(), '<a href="https://www.mediawiki.org/wiki/Manual:index.phtml">index.phtml</a>', 'External link works correctly in parse mode' );
