( function ( mw ) {
	QUnit.module( 'mediawiki.Uri', QUnit.newMwEnvironment( {
		setup: function () {
			...
			mw.Uri = mw.UriRelative( 'http://example.org/w/index.phtml' );
		},
		teardown: function () {
	...
	QUnit.test( 'Constructor( String[, Object ] )', function ( assert ) {
		...
		uri = new mw.Uri( 'http://example.com/index.phtml?key=key&hasOwnProperty=hasOwnProperty&constructor=constructor&watch=watch' );
		assert.deepEqual(
			uri.query,
			{
				key: 'key',
	...
	QUnit.test( 'Constructor( empty[, Object ] )', function ( assert ) {
		...
		testuri = 'http://example.org/w/index.phtml?a=1&a=2';
	...
	QUnit.test( '.getQueryString()', function ( assert ) {
		var uri = new mw.Uri( 'http://loadPage.example.com/?q=uri' );
		...
		uri = new mw.Uri( 'https://example.com/mw/index.phtml?title=Sandbox/7&other=Sandbox/7&foo' );
		assert.equal(
			uri.getQueryString(),
	...
	QUnit.test( '.clone()', function ( assert ) {
		...
		original = new mw.Uri( 'http://foo.example.org/index.phtml?one=1&two=2' );
	...
	QUnit.test( 'Variable defaultUri', function ( assert ) {
		var uri,
			href = 'http://example.org/w/index.phtml#here',
